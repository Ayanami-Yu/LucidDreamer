    ...
    def add_noise_with_cfg(
        ...
        for i in range(inv_steps):
            ...
            if cfg > 1.0:
                ...
                unet_output = unet(
                    cur_noisy_lat_,
                    timestep_model_input,  # 输入 x_{cur_t} 后预测的噪声
            ...
            ).prev_sample  # prev_sample: 预测的 x_{next_t}
            ...
            if (
                cur_ind_t == ind_t
            ):  # 对 t ddim_step 之后就 break，因此 pred_scores 最大只添加到 s
                break
    ...
    def train_step_perpneg(
        ...
        with torch.no_grad():
            ...
            noise_pred_uncond, noise_pred_text = unet_output[:1].reshape(
                ...
                resolution[0] // 8,
                resolution[1] // 8,
            )  # uncond: [2, 4, 64, 64] text: [6, 4, 64, 64]

            delta_noise_preds = noise_pred_text - noise_pred_uncond.repeat(K, 1, 1, 1)
            delta_DSD = weighted_perpendicular_aggregator(delta_noise_preds, weights, B)

        pred_noise = (
            noise_pred_uncond + guidance_opt.guidance_scale * delta_DSD
        )  # ISM Eq.(18)
