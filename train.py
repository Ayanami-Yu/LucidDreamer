    ...
    for i in range(K):
        for text_z in text_z_list:
            ...
            text_embeddings.append(
                text_z[i] if i < len(text_z) else text_z[0]
            )  # [77, 1024]
...
def prepare_embeddings(guidance_opt, guidance):
    ...
    embeddings["inverse_text"] = guidance.get_text_embeds(
        guidance_opt.inverse_text
    )  # [1, 77, 1024]
    return embeddings
...
def guidance_setup(guidance_opt):
    if guidance_opt.guidance == "SD":
        ...
        guidance = StableDiffusion(
            ...
            guidance_opt.fp16,
            guidance_opt.vram_O,  # TODO track guidance_opt
